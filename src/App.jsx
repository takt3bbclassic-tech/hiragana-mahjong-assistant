...
function getWordPlansFromTiles(tiles, dictionary) {
  ...
  const addPlan = (words, kind) => {
    ...
    for (const word of words) {
      const next = consumeWordChars(remaining, word);
      if (!next) return;
      remaining = next;
    }
    ...
    const score = Math.max(...words.map((word) => word.length)) * 30 + longCount * 12 + words.length * 8 + used;
    plans.push({ words, remaining, used, score, kind });
  };
  base.slice(0, 16).forEach((entry) => addPlan([entry.word], "単独"));
  for (let i = 0; i < Math.min(base.length, 14); i += 1) {
    for (let j = i + 1; j < Math.min(base.length, 18); j += 1) {
      addPlan([base[i].word, base[j].word], "2候補");
    }
  }
  for (let i = 0; i < Math.min(base.length, 8); i += 1) {
    for (let j = i + 1; j < Math.min(base.length, 10); j += 1) {
      for (let k = j + 1; k < Math.min(base.length, 12); k += 1) {
        addPlan([base[i].word, base[j].word, base[k].word], "3候補");
      }
    }
  }
  return plans.sort((a, b) => b.score - a.score || a.words.length - b.words.length || a.remaining.length - b.remaining.length || a.words.join("").localeCompare(b.words.join(""), "ja")).slice(0, 12);
}
